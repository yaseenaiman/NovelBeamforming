...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% This algorithm is the basis for this paper 
...
NtRf = 4;%number of transmitter RF chains
...
Elevation_Angle =50; %Elevation Angle (Look Angle)
fc=28e9; % Carrier Frequency 
h = 550e3;           % Satellite Altitude in meters
...
for iter=1:1:ITER
    ...
    [H,a_TX,a_RX]=generate_channels(Num_users,TX_ant_w,TX_ant_h,RX_ant_w,RX_ant_h,Num_paths,Elevation_Angle, fc, h); 
...
%Plotting the spectral efficiency
    ...
    legend('Single-user (No Interference)','MMSE Fully-Digital Precoding','PE-AltMin','Novel Algorithm','OMP','Kalman Hybrid Precoding','MMSE Hybrid Precoding','ZF Hybrid Precoding','Analog-only Beamsteering');
...
function [H,a_TX,a_RX]=generate_channels(Num_users,TX_ant_w,TX_ant_h,RX_ant_w,RX_ant_h,Num_paths,Elevation_Angle,fc, h)
...
for u=1:1:Num_users
    ...
    [f_doppler, distance, ~] = doppler_generation_NovelAlgorithm(Elevation_Angle,fc,h);
    % Calculate the propagation delay based on altitude and speed of light
propagation_delay = (1/(3e8))*distance;
...
real_part =   A+ sigmaR * randn(Num_paths, 1);
...
alpha(u,:) = (real_part + 1j * imaginary_part);
 ...
 Temp_Channel=zeros(RX_ant_w*RX_ant_h,TX_ant_w*TX_ant_h);
  ...
  for l=1:1:Num_paths
         ...
         f_doppler_d =  0.99*f_doppler+0.1*randn(1,1) ;


          timeFactor = exp(1j * 2 * pi * abs(f_doppler_d-f_doppler) * abs((tau(l)-propagation_delay))); 
        ...
        Temp_Channel=Temp_Channel+( timeFactor)*sqrt((TX_ant_w*TX_ant_h/Num_paths)*(RX_ant_w*RX_ant_h))*alpha(u,l)*a_RX(:,u)*a_TX(:,u)';
